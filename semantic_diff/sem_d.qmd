...
ggplot(df_vis,aes(mean, y, color = gruppe, group = gruppe)) + 
  ...
  geom_path(position=position_dodge(width=0.2)) + 
  theme_minimal() +
  coord_cartesian(xlim = c(-2,2)) + # bringt x achse in die Mitte 
  geom_vline(aes(xintercept = 0), linetype = "dashed") + 
  scale_y_continuous(breaks = 1:length(lab_l), name = NULL, labels = lab_l,
    ,sec.axis = sec_axis(trans = ~., breaks = 1:length(lab_r), labels = lab_r))+
  theme(
    panel.grid.minor = element_blank(),
    legend.position = "top",
    legend.title = element_blank(),
    axis.text.y = element_text(face = "bold", size = 1)
    )
